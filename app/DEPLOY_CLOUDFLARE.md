...
- Wrangler:
  - `app/wrangler.toml` → `database_id` toma `D1_DATABASE_ID`
...
3. Pull Request a `main` para revisión.
4. Merge a `main`.
5. GitHub Actions:
   - ejecuta `npm ci`, `npm run build`, `npm run lint`
   - despliega con `wrangler pages deploy dist --project-name=yosoymx --branch=main`
   - desde local (verificación puntual): `cd app && npx wrangler pages deploy dist --project-name yosoymx --branch main`

## 3) Configuración en Cloudflare Pages
1. En el panel de Pages, conecta el repositorio de GitHub y rama `main`.
2. Verifica:
...
3. Configura `D1_DATABASE_ID` y confirma que `app/wrangler.toml` no conserve placeholders.
