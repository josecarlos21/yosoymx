...
- `xcodebuild -project GacetaIOS.xcodeproj -scheme GacetaIOS -destination 'generic/platform=iOS' -archivePath /tmp/GacetaIOS-release-v1.0.0-3.xcarchive archive`
- `bash ios/scripts/prepare-testflight-release.sh` para incrementar build y regenerar archive
- `bash release/app-store/scripts/capture-ios-screenshots.sh` para refrescar storefront antes de subir
...
- `https://yosoymx.com/.well-known/apple-app-site-association` responde `200` JSON sin challenge
...
4. Verificar edición vigente, archivo por universal link y comunidad real desde un iPhone con la API ya liberada en Cloudflare.
...
- Si `/.well-known/apple-app-site-association` sigue bajo challenge o redirect, los universal links no cerrarán.
